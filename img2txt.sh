...
if [ $engine = "-k" ]; then
	if [ ! -d $SCRIPT_DIR/venv_kraken/ ]; then
		echo "Création de l'environnment virtuel pour accueillir Kraken."
		mkdir $SCRIPT_DIR/venv_kraken/
		python3 -m venv $SCRIPT_DIR/venv_kraken/
		. $SCRIPT_DIR/venv_kraken/bin/activate
	...
	if [ ! -d $SCRIPT_DIR/venv_tesseract/ ]; then
		echo "Création de l'environnment virtuel pour accueillir Tesseract."
		mkdir $SCRIPT_DIR/venv_tesseract/
		virtualenv -p python3 venv_tesseract
		. $SCRIPT_DIR/venv_tesseract/bin/activate
...
if [ $engine = "-k" ]; then
	if [ $outType = "-t" ]; then
		for file in `ls $dir_path/*_bin.png`; do timeout 600 kraken -i $file $file".txt" segment ocr -m $SCRIPT_DIR/CORPUS17.mlmodel; done
	else
		for file in `ls $dir_path/*_bin.png`; do timeout 600 kraken -i $file $file".html" segment ocr -m $SCRIPT_DIR/CORPUS17.mlmodel -h; done
	fi
elif [ $engine = "-t" ]; then
